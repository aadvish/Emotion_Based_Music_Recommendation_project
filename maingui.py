...
def showplaylist(textbox1, textbox2, textbox3):
    ...
    # Start a new thread to execute the show_playlist_web_page function
    playlist_thread = Thread(target=show_playlist_web_page, args=(artist_name, emotion, textbox3))
...
def ask_detect_again(textbox2, emotion):
    ...
    def no_action(textbox2, emotion):
        popup.destroy()
        if emotion not in ["happy","calm"]:
            choicepopup(textbox2)

    label1 = CTkLabel(master=popup, text="Detected Emotion: " + emotion)
    label1.place(relx=0.5, rely=0.1, anchor="n")
    
    yes_btn = CTkButton(popup, text="No, detect again", command=yes_action)
    yes_btn.place(relx=0.5, rely=0.4, anchor="n")

    no_btn = CTkButton(popup, text="Yes", command=lambda: no_action(textbox2, emotion))
...
def choicepopup(textbox2):
    ...
    yes_btn = CTkButton(popup, text="Need to uplift", command=yes_action)
    yes_btn.place(relx=0.5, rely=0.4, anchor="n")

    no_btn = CTkButton(popup, text="Stay in the same emotion", command=no_action)
    no_btn.place(relx=0.5, rely=0.8, anchor="n")
    
    popup.mainloop()

def show_playlist_web_page(artist_name, emotion, textbox3):
    try:
        # Call the classify_songs function to get the list of songs
        ...
        # Update the textbox with the status
        textbox3.delete("1.0", "end")
        textbox3.insert("1.0", "Songs Generated Successfully.....")
    except Exception as e:
        # Update the textbox with the error message if any
        textbox3.delete("1.0", "end")
        textbox3.insert("1.0", f"Error: {str(e)}")
